...
pub fn get_opts(mode: CdMode, exact: bool, mock: Option<OsString>) -> SearchOptions {
    SearchOptions {
        mode,
        exact,
...
pub fn get_opts_fuzzy(mode: CdMode, exact: bool, mock: Option<OsString>) -> SearchOptions {
    SearchOptions {
        mode,
        exact,
...
pub fn setup_test_env() -> (tempfile::TempDir, PathBuf) {
...
mod local_battery_1 {
    ...
    use std::path::{Path, PathBuf};
    use crate::unit_tests_local::{get_opts, get_test_root, setup_test_env, test_opts, CwdGuard};
